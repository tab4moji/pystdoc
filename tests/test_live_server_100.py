...
class TestLiveServer100(unittest.TestCase):
    ...
    def test_pass1_function_symbol_top_down_context_retention_and_format(self):
        c_file = self.src_dir / "calc_app.c"
        c_file.write_text(
            "int main() {\n    return 0;\n}\n",
            encoding="utf-8",
        )
        ...
        doc_content = main_doc_path.read_text(encoding="utf-8")

        # 1. Must contain Basic Information section
        self.assertIn("## 1. Basic Information", doc_content)
        # 2. Must contain the role generated by LLM (from MockLLMHandler)
        self.assertIn("Main computation unit", doc_content)
        # 3. Must NOT contain the old superficial fallback template
